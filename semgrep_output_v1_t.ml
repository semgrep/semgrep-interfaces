...
*)
type match_severity = [
    `Error | `Warning | `Experiment | `Inventory | `Critical | `High
  | `Medium | `Low | `Info
]
  [@@deriving eq, ord, show]
...
type engine_of_finding = [ `OSS | `PRO | `PRO_REQUIRED of pro_feature ]
  [@@deriving ord, show]
...
type very_slow_stats = { time_ratio: float; count_ratio: float }
...
type project_root = [ `Filesystem of string | `Git_remote of string ]
  [@@deriving show]
...
type product = [ `SAST | `SCA | `Secrets ] [@@deriving eq, ord, show]
...
type error_severity = [ `Error | `Warning | `Info ] [@@deriving show, eq]
...
type symbol_usage = { symbol: symbol; locs: location list } [@@deriving show]

type symbol_analysis_upload_response = { upload_url: uri }

type symbol_analysis = symbol_usage list [@@deriving show]

type resolution_method = [ `LockfileParsing | `DynamicResolution ]
  [@@deriving show]
...
type skipped_rule = { rule_id: rule_id; details: string; position: position }
...
type action = [ `Message of string | `Delay of float | `Exit of int ]
...
type profiling_entry = { name: string; total_time: float; count: int }
...
type diff_file = { filename: fpath; diffs: string list; url: string }
  [@@deriving show]
